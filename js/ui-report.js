...
export function buildFullCSV(S) {
  ...
  R.push(['# PERIOD SUMMARY (per owner)']); R.push(['Period', 'Kind', 'Owner', 'Logged', 'Weighted', 'Avg weight', 'Role', 'Salary EUR', 'Paid hours', 'Paid weighted', 'Unpaid hours', 'Equity hours', 'Share this period %', 'Cumulative equity hours', 'Cumulative share %', 'Column status', 'Approved by this owner']);
  C.displayRows(S).forEach(r => owners.forEach(o => { const k = comp[r.id].cells[o.id], c = C.cellOf(r, o.id); const st = C.colFinal(S, r) ? 'finalised' : (r.audit.needsReapproval ? 're-approval needed' : `approved ${C.colApproved(S, r)}/${owners.length}`);
    R.push([r.label, r.kind, o.name, fmt(k.H), fmt(k.W), fmt(k.avgW), C.roleName(S, c.role), c.salary != null ? fmt(c.salary) : '', fmt(k.paidHours), fmt(k.paidW), fmt(k.unpaidHours), fmt(k.equity), fmt(k.monthly), fmt(comp[r.id].cum[o.id]), fmt(comp[r.id].cumShare[o.id]), st, r.approvals[o.id] ? 'yes' : 'no']); })); R.push([]);
...
function pdfOwnerCard(S, r, o, k, c) {
  ...
  return `<div class="pcard"><div class="ph"><span class="sw" style="background:${C.ownerColor(S, o.id)}"></span><b>${esc(o.name)}</b><span class="st">${r.approvals[o.id] ? '✓ approved' : 'not approved'}</span></div>
