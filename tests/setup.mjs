...
export async function setup(o = {}) {
  const {user = 'jal@example.com', example = true, tables, url = SITE, boot = true} = o;
  ...
  const sb = createMockSupabase({tables: tables || makeDb({example}), users: USERS.map(u => ({...u})), session: user ? USERS.find(u => u.email === user) : null});
  const [app, D, A, C, tracker, report, owners, tasks] = await Promise.all([
    import('../js/app.js'), import('../js/data.js'), import('../js/auth.js'), import('../js/calc.js'), import('../js/ui-tracker.js'), import('../js/ui-report.js'), import('../js/ui-owners.js'), import('../js/ui-tasks.js')]);
  ...
  return {dom, window: w, document: w.document, sb, app, D, A, C, tracker, report, owners, tasks, env, $, $$, fire, setValue, flush, text, USERS};
}
